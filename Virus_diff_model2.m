...
function u0 = pdeic(x,P) % inital conditions
    ...
    % Experiment 2: Fitting model to data. Run Mckee_nonlin_fitter. We also use Total_pop_cal.
    % Important, coordiantes in spherical geometry, set m=2 above
    % Initial conditions for virus treament with collagenase
    %coll = (0.15*sin(5*x)+0.75);
    %uu = 10^6*(1-coll);%.*(heaviside(3-x)) 
    %vv = 2.4*10^5*heaviside(1-x);
    %col = coll;  We multiply this by P(2) when fitting to data.
    
     
     coll = 0.85.*exp(-0.4568.*x); %exp(-P(11).*x);%  This is used for the collagen fit to data.
     %coll = (0.15*sin(5*x)+0.75);% high collagen
     
     % simulate desmoplastic reaction for ueno simulations
    
     %col = -1*(square(1*x+2,5));   % immature collagen
     %col = -1*(square(3*x,12)); % Intermediate collagen
     %col = -1*(square(10*x)); % Mature collagen
     %coll = (col+1.1)/2.5; % rescale the collagen for the different cases
     
     % simulate desmoplastic reaction for varying collagen density mature
     % and immature
     
     %col = -1*(square(10*x)); %P = [5 0.64;10 0.74] input this in sensitivity analysis
     %coll = col/P(1)+P(2);
     %col = -1*(square(1*x+2,5));
     %coll = col/P(1)+P(2);
     
     uu = 10^6*(1-coll);%.*(heaviside(3-x)); %use heaviside when fitting data (r_0=3)
     
     II = 0; 
     
     col = coll*1; % This is used when fitting model to data for high(1)and low collagen(0.1)exponential case
     %col = coll*0.83;  This is used when fitting model to data for high scale by 0.9 and low scale by 1/3. For local sensitivity we use 0.83 and 1/3 for high and low collagen respectively. 
     %For r_u in high collagen, we allow for very small diffusion of collagen 0.001 to avoid numerical error.
